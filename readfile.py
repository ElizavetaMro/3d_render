...
from model import Model, ModelLight
import numpy as np
import time
import re
...
class Data:
    ...
    def __init__(self, file):
        """Содержит список объектов Model или ModelLight"""
        self.model_list = self.parse_wrl(file)
        
    def parse_wrl(self, file_path):
        """Проверяет к какому типу относится файл TFlex или Solid
            Вызывает соответсвующую функцию чтения"""
    ...
    def parse_wrl_Solid(self, file_path):
        """Парсит wrl Solid файл
        Записывает вершины, индексы и цвета в Model
        Возвращает список объектов Model  
        """
        with open(file_path, 'r') as f:
            lines = f.readlines()
        ...
        for line in lines:
            ...
            if flag_index == 1:
                if line == ']':
                    ...
                    model_list += [Model(vertices = points_list , indices = indexs_list, color=color)]
                    continue
                indexs = ' '.join([indexs, line])
        return model_list
    
    def parse_wrl_TFlex(self, file_path):
        """Парсит wrl TFlex файл
        Записывает вершины, индексы и цвета в Model
        Возвращает список объектов Model  
        """
        with open(file_path, 'r') as f:
            ...
            if indexs == '':
                ...
                if flag_index == 1:
                    if line[-1] == ']':
                        ...
                        points_list = np.fromstring(points.replace(',', ' '), sep=' ', dtype=np.float32).reshape(-1,3)
                        indexs_list =np.fromstring( indexs.replace(',-1', ''), sep=',', dtype=np.float32)
                        model_list += [Model(vertices=points_list , indices=indexs_list, color=color, transform=transform_dict)]  
        return model_list


class DataLight(Data):

    def parse_wrl_Solid(self, file_path):
        """Парсит wrl Solid файл
        Записывает вершины, индексы, цвета и нормали в ModelLight
        Возвращает список объектов ModelLight 
        """
        with open(file_path, 'r') as f:
            lines = f.readlines()
        ...
        for line in lines:
            ...
            if flag_index_normal == 1:
                if line == ']':
                    ...
                    indexs_list = np.fromstring( indexs.replace(', -1', ''), sep=', ', dtype=np.float32)
                    indexs_normal_list = np.fromstring( indexs_normal.replace(', -1', ''), sep=', ', dtype=np.float32)
                    model_list += [Model(points_list, indexs_list, color = color)] #Пока не реализована работа со светом в Solid
                    continue
                indexs_normal = ' '.join([indexs_normal, line])   
        return model_list
            ...
            if normal == '':
                ...
                if flag_normal == 1:
                    if line[-1] == ']':
                        ...
                        normal_list = np.fromstring(normal.replace(',', ' '), sep=' ', dtype=np.float32).reshape(-1,3)
                        indexs_list =np.fromstring( indexs.replace(',-1', ''), sep=',', dtype=np.float32)
                        model_list += [ModelLight(points_list, indexs_list, normal_list, color = color, transform = transform_dict)]  
        return model_list
...
if __name__ == "__main__":
    ...
    data = DataLight("wrls\\Detal_2.wrl")
